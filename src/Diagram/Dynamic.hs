...
-- elements.
withCapacity :: (PrimMonad m, MVector v a) => Int -> m (Dynamic (PrimState m) v a)
withCapacity = fmap (Dynamic 0) . MV.new
{-# INLINE withCapacity #-}

-- | Create an empty vector
new :: (PrimMonad m, MVector v a) => m (Dynamic (PrimState m) v a)
new = withCapacity 0
{-# INLINE new #-}

-- | Clone the given vector
clone :: (PrimMonad m, MVector v a) => Dynamic (PrimState m) v a -> m (Dynamic (PrimState m) v a)
...
pop :: (PrimMonad m, MVector v a) =>
       Dynamic (PrimState m) v a -> m (Maybe a, Dynamic (PrimState m) v a)
...
toList :: (G.Vector v a, PrimMonad m) => Dynamic (PrimState m) (G.Mutable v) a -> m [a]
toList = MV.foldr' (:) [] . dynVector
{-# INLINE toList #-}

-- | Create a Dynamic vector from a list
fromList :: (G.Vector v a, PrimMonad m) =>
            [a] -> m (Dynamic (PrimState m) (G.Mutable v) a)
...
read :: (PrimMonad m, MVector v a) => Dynamic (PrimState m) v a -> Int -> m a
read = MV.read . dynVector
{-# INLINE read #-}

-- | Read an element at the given position.
(!) :: (PrimMonad m, MVector v a) => Dynamic (PrimState m) v a -> Int -> m a
(!) = read
{-# INLINE (!) #-}
infixl 9 !
...
mapM_ :: (PrimMonad m, MVector v a) => (a -> m b) -> Dynamic (PrimState m) v a -> m ()
...
imapM_ :: (PrimMonad m, MVector v a) => (Int -> a -> m b) -> Dynamic (PrimState m) v a -> m ()
imapM_ f = MV.imapM_ f . fromDynamic
{-# INLINE imapM_ #-}

-- | O(n) Apply the monadic action to every element of the vector,
-- discarding the results.  It's the same as flip mapM_.
forM_ :: (PrimMonad m, MVector v a) => Dynamic (PrimState m) v a -> (a -> m b) -> m ()
forM_ = flip mapM_
{-# INLINE forM_ #-}

-- | O(n) Apply the monadic action to every element of the vector and
-- its index, discarding the results.  It's the same as flip imapM_.
iforM_ :: (PrimMonad m, MVector v a) => Dynamic (PrimState m) v a -> (Int -> a -> m b) -> m ()
iforM_ = flip imapM_
{-# INLINE iforM_ #-}
...
foldl :: (PrimMonad m, MVector v a) =>
         (b -> a -> b) -> b -> Dynamic (PrimState m) v a -> m b
...
foldM :: (PrimMonad m, MVector v a) =>
         (b -> a -> m b) -> b -> Dynamic (PrimState m) v a -> m b
...
foldr :: (PrimMonad m, MVector v a) =>
         (a -> b -> b) -> b -> Dynamic (PrimState m) v a -> m b
...
foldrM :: (PrimMonad m, MVector v a) =>
          (a -> b -> m b) -> b -> Dynamic (PrimState m) v a -> m b
...
ifoldl :: (PrimMonad m, MVector v a) =>
          (b -> Int -> a -> b) -> b -> Dynamic (PrimState m) v a -> m b
...
ifoldM :: (PrimMonad m, MVector v a) =>
          (b -> Int -> a -> m b) -> b -> Dynamic (PrimState m) v a -> m b
...
ifoldr :: (PrimMonad m, MVector v a) =>
          (Int -> a -> b -> b) -> b -> Dynamic (PrimState m) v a -> m b
...
ifoldrM :: (PrimMonad m, MVector v a) =>
           (Int -> a -> b -> m b) -> b -> Dynamic (PrimState m) v a -> m b
